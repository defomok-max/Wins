...
**重点**：默认模式下 WindsurfAPI 只负责**传递** tool_use / tool_result，真正改文件的是客户端 CLI。

### 可选：服务端 Agent Loop

如果你想让模型在一次请求里像完整 agent 一样连续使用工具，可以显式开启服务端执行循环。代理会自动补充内置工具（`Bash` / `Read` / `Write` / `Edit` / `MultiEdit` / `LS` / `Grep` / `Glob` / `WebFetch`），执行模型返回的 tool call，把 tool result 注入下一轮，再继续请求模型直到最终回答或达到安全上限。即使模型用 `<tool_call>...</tool_call>` / JSON 文本格式“发出”工具调用，Agent Loop 也会把它解析成真实 tool call 并在服务器上实际执行，而不是把模拟文本原样返回。
...
```json
{
  ...
  "agent": { "enabled": true, "max_iterations": 8, "working_directory": "/path/to/workspace" },
  "messages": [{ "role": "user", "content": "Read package.json and summarize scripts" }]
}
```

也可以用 `"tool_execution": "server"` / `"agent_mode": true`，或设置 `WINDSURFAPI_AGENT_LOOP=1` 全局开启。默认工作目录是进程 cwd，可用 `WINDSURFAPI_AGENT_WORKDIR` 覆盖；文件工具不能逃出该目录。
...
**Q: 模型说"我无法操作文件系统"**
A: 默认模式下这是 **chat API**，不是 IDE agent。要让模型真的改文件，推荐用 **Claude Code / Cline / Cursor / Aider** 之类的客户端 CLI，把它们的 API base URL 指向本服务；模型出 tool_use，客户端本地执行，再把 tool_result 发回来。也可以开启上面的服务端 Agent Loop，让 WindsurfAPI 在配置的工作目录里执行内置工具并自动循环到最终回答。
